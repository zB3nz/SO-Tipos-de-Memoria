...
namespace MemoriaParticionadaDinamicaReubicacion
{
    class Particion
    {
    ...
    class Program
    {
        ...
        static double TamMemoriaN = 0, noTrabajoSiguiente = 1, divisor = 20;
        static int unidades;
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Bienvenido al programa simulador de Memoria Particionada Dinámica.");
            Console.ForegroundColor = ConsoleColor.Yellow;
            Console.WriteLine("//////////////////////////////////////////////////////////////////");
        ...
        private static void PreguntarTamaños()
        {
            ...
            try
            {
                ...
                ParticionSO.SO = true;
                ParticionSO.Posicion = 0;
                Particiones.Add(ParticionSO); //automatica el SO se convierte en Particiones[0]

                Particion memoriaRestante = new Particion((int)TamMemoriaN - TrabajoSO.Tamano);
                memoriaRestante.Posicion = 1;
                Particiones.Add(memoriaRestante);
        ...
        private static void PreguntarTrabajos()
        {
            ...
            while (loop)
            {
                ...
                try
                {
                    ...
                    //se trata que quiere dar de baja
                    if (txtIngresarTrabajo.Contains("#"))
                    {
                        ...
                        //en caso de que elimine del standby
        ...
        private static void AsignarTrabajo(int memoriaTrabajo)
        {
            ...
            //Saber si cabrá en la memoria (la ec. es memoriatotal menos memoria SO)
            if (trabajo.Tamano > TamMemoriaN - Particiones[0].Tamano)
            {
            ...
            //Alta - primero buscamos la partición menor que acepte al trabajo
            Particion menor = new Particion(int.MaxValue);
            foreach (Particion p in Particiones)
            {
                if (p.Tamano < menor.Tamano && p.Tamano >= memoriaTrabajo && p.EstaVacia()) menor = p;
            }

            if (menor.Tamano != int.MaxValue)
            {
                menor.Trabajo = trabajo;
                CortarParticion(menor);
            ...
            //Verificar si se puede reubicar
            if (trabajo.Tamano <= ContarEspacioLibre())
            {
                Aviso("El trabajo #" +trabajo.NoTrabajo+ " no puede ser atendido. Se hará una reubicación para atenderlo.");
                Console.ReadKey();

                Reubicacion();
                ActualizarTabla();
                Aviso("La memoria ha sido reubicada. Ahora se ingresará el trabajo #"+trabajo.NoTrabajo);
                Console.ReadKey();

                noTrabajoSiguiente--;
                AsignarTrabajo(memoriaTrabajo);
        ...
        private static void Reubicacion()
        {
            int espacioLibre = ContarEspacioLibre();
            int ultimaPosicion = 0;

            foreach (Particion p in ParticionesEnumeradas())
            {
                ultimaPosicion = p.Posicion;

                if (p.EstaVacia())
                {
                    RecorrerParticiones(p.Posicion, -1);
                    Particiones.Remove(p);
            ...
            //por si sale mal
            if (ultimaPosicion == 0) throw new Exception();

            Particion libre = new Particion(espacioLibre);
            libre.Posicion = ultimaPosicion + 1;
            Particiones.Add(libre);
        }

        private static int ContarEspacioLibre()
        {
            int espacioLibre = 0;

            foreach (Particion p in ParticionesEnumeradas())
                if (p.EstaVacia())
                    espacioLibre += p.Tamano;
        ...
        private static void Compactar()
        {
            foreach (Particion p in ParticionesEnumeradas())
            {
                ...
                Particion next = LlamarParticion(p.Posicion + 1);

                if (next == null) return;
                if (!next.EstaVacia()) continue;

                p.Tamano += next.Tamano;
                next.Tamano = 0;
                RecorrerParticiones(p.Posicion + 1, -1);

                Particiones.Remove(next);
                Compactar();

                ActualizarTabla();
                Aviso("La memoria ha sido compactada.");
                Console.ReadKey();
                break;
            }
        }

        private static void CortarParticion(Particion menor)
        {
            int tamResParticion = menor.Tamano - menor.Trabajo.Tamano;
            menor.Tamano = menor.Trabajo.Tamano;
            if (tamResParticion == 0) return;

            Particion nuevaParticion = new Particion(tamResParticion);
            nuevaParticion.Posicion = menor.Posicion + 1;
            RecorrerParticiones(menor.Posicion, 1);

            Particiones.Add(nuevaParticion);
        }

        private static void RecorrerParticiones(int posicion, int recorrido)
        {
            for (int i = posicion + 1; i < Particiones.Count; i++)
                LlamarParticion(i).Posicion += recorrido;
        }

        private static Particion LlamarParticion(int no)
        {
            foreach (Particion p in ParticionesEnumeradas())
                if (p.Posicion == no) return p;

            return null;
        }

        private static IEnumerable<Particion> ParticionesEnumeradas()
        {
            int noParticiones = Particiones.Count;
            for (int i = 0; i < noParticiones; i++)
                foreach (Particion p in Particiones)
                    if (p.Posicion == i)
                    {
                        yield return p;
        ...
        private static void ActualizarStandBy()
        {
            ...
            foreach (Trabajo t in StandBy)
            {
                //buscamos la partición menor que acepte al trabajo
                Particion menor = new Particion(int.MaxValue);
                foreach (Particion p in Particiones)
                {
                    if (p.Tamano < menor.Tamano && p.Tamano >= t.Tamano && p.EstaVacia()) menor = p;
                }
                ...
                //si llega acá es porque no hubo espacio. Comprobar si se puede reubicar
                //Verificar si se puede reubicar
                if (t.Tamano <= ContarEspacioLibre())
                {
                    Aviso("El trabajo #" + t.NoTrabajo + " puede ser atendido. Se hará una reubicación para atenderlo.");
                    Console.ReadKey();

                    Reubicacion();
                    ActualizarTabla();
                    Aviso("La memoria ha sido reubicada. Ahora se ingresará el trabajo #" + t.NoTrabajo);
                    Console.ReadKey();

                    //agregamos el trabajo
                    Particion libre = null;

                    foreach (Particion p in ParticionesEnumeradas()) //el ultimo trabajo es el libre
                        libre = p;

                    libre.Trabajo = t;
                    StandBy.Remove(t);
                    CortarParticion(libre);

                    ActualizarTabla();
                    Aviso("Se ha añadido el trabajo #" + t.NoTrabajo + " (" + ConvertirKBytesAUnidades(t.Tamano) + " " + txtUnidades + ").");
                    Console.ReadKey();
        ...
        private static void ActualizarTabla()
        {
            ...
            foreach (Particion p in ParticionesEnumeradas())
            {
                ...
                for (int i = 1; i < renglonesParticion; i++)
                {
                    ...
                    {
                        if (p.Trabajo == null) Console.Write("║\tMemoria Libre ("
                            + ((unidades != 1) ? ConvertirKBytesAUnidades(p.Tamano).ToString("N2") : p.Tamano.ToString())
                            + " " + txtUnidades + ")");
                        if (p.Trabajo != null)
                        {
                            Console.Write("║\tTrabajo #" + p.Trabajo.NoTrabajo + " (" + ConvertirKBytesAUnidades(p.Trabajo.Tamano) + " " + txtUnidades + ")");
                            memoriaDesperdiciada += (p.Tamano - p.Trabajo.Tamano);
                        }
                        WriteAt("║ #" + p.Posicion, 40, Console.CursorTop);
                        Console.WriteLine();
                        continue;
                    }
